...
static constexpr unsigned long XOJ_EXCEPTION_ABORT = 0x0001;
...
static long __stdcall win32SehCrashHandler(LPEXCEPTION_POINTERS ex);
...
void installCrashHandlers() {
    ...
    signal(
            SIGABRT, +[](int) {
                ...
                if (!crashing) {
                    crashing = true;
                    RaiseException(XOJ_EXCEPTION_ABORT, EXCEPTION_NONCONTINUABLE, 0, nullptr);
                }
            });
}

// clang-format off
constexpr std::array ect =
        {
            std::pair{XOJ_EXCEPTION_ABORT, "Abort"},
            
            std::pair{EXCEPTION_ACCESS_VIOLATION, "Access violation"},
...
public:
    ...
    CrashHandlerCommon(const char* reason, Dumplog&& d): dumplog_(new Dumplog(std::move(d))) {
        if (alreadyCrashed_) {
            exit(2);
        }
...
private:
    std::unique_ptr<Dumplog> dumplog_;
};


long __stdcall win32SehCrashHandler(LPEXCEPTION_POINTERS ex) {
    {
